$(document).ready(function () {
    ...
    function encryptUserCode(userCode) {
        return btoa(userCode); // Convert user code to Base64
    }
    
    // Decrypt function
    function decryptUserCode(encryptedCode) {
        try {
            return atob(encryptedCode); // Convert from Base64 to the original user code
        } catch (e) {
            showAlertError("Usercode not matched");
        }
    }
    ...
    $("#formAuthentication").submit(function (e) { 
        ...
        localStorage.setItem('payroll_token', sessionToken);
    
        var maincode = $("#user_code").val();
        // var encryptedUserCode = encryptUserCode(user_code);
        // console.log("Encrypted User Code:", encryptedUserCode);

        var user_code = decryptUserCode(maincode);
        // console.log("decrypted User Code:", decryptUser);

        var password = $("#password").val();
    
        // Validate input fields
        if (user_code.trim() === "" || password.trim() === "") {
        ...
        $.get('https://api.ipify.org?format=json', function(data) {
            ...
            $.ajax({
                type: "POST",
                url: "login_handle.php", // Replace with your server endpoint
                data: formData,
                success: function(response) {
                    if (response === "success") {
                        showAlertSuccess("Login successful.");
                        window.location.href = "dashboard?ss_id=" + sessionToken;
                    } else {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response === "success") {
                    ...
                    window.location.href = "list_jobs?ss_id=" + localStorage.getItem('payroll_token');
                } else {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response === "success") {
                    ...
                    window.location.href = "job_status?ss_id=" + localStorage.getItem('payroll_token');
